...
export default function ImageSection({ images }: { images: Array<string> }) {
    ...
    const handlers = useSwipeable({
        onSwipedLeft: () => handleImgIndex("right"),
        onSwipedRight: () => handleImgIndex("left"),
    ...
    return (
        <div id="images" className='pt-4 flex flex-col md:flex-row-reverse gap-4 md:col-span-2'>
            <div className='h-[350px] md:h-[420px]' {...handlers}>
                <div className="relative flex items-center justify-center select-none h-full">
                    ...
                    <Image
                        ...
                        width={350}
                        height={350}
                        className='border border-primary/20 rounded-lg h-full shadow-lg shadow-primary/30 transition-transform duration-300 ease-in-out'
                    />
                ...
                {images.map((img: string, index: number) => {
                    if (index < 4)
                        return (
                            <div
                                key={index}
                                className='h-[80px] md:h-[100px] cursor-pointer'
